...
<div class="table-responsive">
	<table class="table table-hover table-stiped">
		...
		if ($post!="") {
			extract($post);
			$res = $base->select("user u, level l, gender g, agama a, sts_perkawinan s where u.nik LIKE '%$search%' and u.level=l.id and u.gender=g.id and u.agama=a.id and u.sts_perkawinan=s.id and u.level='3 and 4' and status = 'active'","u.nik, u.name, u.email, g.gender as gender, u.tmp_lahir, u.tgl_lahir, a.agama, s.nama as status, u.pekerjaan, u.alamat, u.phone, l.name as level");
		}else{
			$res = $base->select("user u, level l, gender g, agama a, sts_perkawinan s where u.level=l.id and u.gender=g.id and u.agama=a.id and u.sts_perkawinan=s.id and u.level='3 and 4' and status = 'active'","u.nik, u.name, u.email, g.gender as gender, u.tmp_lahir, u.tgl_lahir, a.agama, s.nama as status , u.pekerjaan, u.alamat, u.phone, l.name as level");
		}
		$no=1;
